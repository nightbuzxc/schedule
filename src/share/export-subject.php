...
function exportPDF($groupName, $data) {
    ...
    $pdf->Cell(0, 10, "Розклад предметів: $groupName", 0, 1, 'C');
    ...
    $tbl .= '<tr>
        ...
        <td>' . htmlspecialchars($row['teachers']) . '</td>
...
function exportExcel($groupName, $data) {
    ...
    $sheet->setCellValue('A1', "Розклад предметів: $groupName");
    $sheet->fromArray(['День', '№ пари', 'Тип', 'Предмет', 'Викладач', 'Кабінет'], NULL, 'A2');
...
foreach ($data as $row) {
    ...
    $sheet->setCellValue("E$rowNum", $row['teachers']);
